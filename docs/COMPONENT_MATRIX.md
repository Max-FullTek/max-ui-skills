...
This inventory classifies the 14 components currently implemented in `sample-orange-matters/src/components/` plus the `DashboardFrame` layout represented by `App.tsx`/`App.module.scss` and its recipe. It is the source of truth for component sharing, recipe coverage, installable asset status, and sample coverage.
...
No installable React assets exist under `skills/orange-matters/assets/react/` yet. Every asset cell below is therefore explicitly planned; the current sample path identifies the extraction source, not a published dependency.
